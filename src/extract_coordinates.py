...
filtered_df = projected_shapefile[projected_shapefile['GRIDCODE'].isin([21, 22])] #TODO: argc argv
...
filtered_df['area_km²'] = filtered_df['geometry'].area / 1e6  # leftshift by 6 to convert to km^2
# Debugging: Print the areas to check if they are calculated correctly
print(filtered_df[['GRIDCODE', 'area_km²']])

filtered_df = filtered_df[(filtered_df['area_km²'] >= 5000)] # filter by size
...
filtered_df = filtered_df.to_crs(epsg=4004) #base map projection, same as 4326
